...
class ORM
{
            ...
            if (is_array($col)) {
                foreach ($col as $key => $value) {
                    if (is_numeric($key)) {
                        ...
                        $this->column($value, $key);
                    }
                }
                return $this;
            } elseif ($col instanceof Expression) {
                $col = $col->sql();
            } else {
                if ($col) {
                    # code...
     ...
     */
    public function join($table, $on, $cols = null)
    {
        if (is_array($table)) {
            $alias = key($table);
            $table = current($table);
            $table_identifier = self::_backQuoteWord($alias);
            $table = self::_backQuoteWord($table).' AS '.$table_identifier;
        } else {
            $table = $table_identifier = self::_backQuoteWord($table);
        }
        if (is_array($on)) {
        ...
        if ($cols !== null) {
            foreach ($cols as $key => $col) {
                if ($col instanceof Expression) {
                    ...
                    $col = self::_backQuoteWord($col);
                }
                if (is_numeric($key)) {
                    $this->_selects[] = $table_identifier.'.'.$col;
                } else {
                    $this->_selects[] = $table_identifier.'.'.$col.' AS '.$key;
                }
            }
        }
        return $this;
    ...
    private function _table($table, $alias = null)
    {
        if ($alias === null) {
            $this->_tables[] = self::_backQuoteWord($table);
        } else {
    ...
    {
        $t = self::_backQuoteWord($this->_table);
        if ($this->_as) {
            $t .= ' AS '.self::_backQuoteWord($this->_as);
        }
        // array_unshift($this->_tables, $t); // ??
        return $t;
    }
